...
parser.add_argument('--beta', type=float, default=100, help='penalty parameter for KL term')
...
#    model.train()
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        # train with fake
        noise = torch.FloatTensor(data.size(0), nz).uniform_(-5, 5).to(device)
        fake = netG(noise)
        vm_fake = fake.var(0)
        
        var_pen = - 10*torch.exp(torch.sqrt(torch.sum(torch.log(vm_fake)**2)))
...
#        errD_fake.backward()
        ...
        errD = errD_real + errD_fake  + var_pen 
        errD.backward()
        optimizerD.step()
